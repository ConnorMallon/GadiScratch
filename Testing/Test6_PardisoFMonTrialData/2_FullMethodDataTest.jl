...
DATAPATH = "tmp/TrialData"
Level_Set_Name = "Distance_map.json"
u_MRI_Name(t) = "u_MRI_$(t)"

#import Level_Set
Level_Set_Path = join([DATAPATH, Level_Set_Name], "/")
Level_Set = JSON.parsefile(Level_Set_Path)
...
#Forcing terms & background grid- TEST VERSION
u(x,t) = 150*VectorValue(x[2],-x[1])*(t/Δt)
p(x,t) = (x[1] - x[2])*(t/Δt)
u(t::Real) = x -> u(x,t)
p(t::Real) = x -> p(x,t)
f(t) = x -> ρ *  ∂t(u)(t)(x) + ρ * conv(u(t)(x),∇(u(t))(x)) - μ*Δ(u(t))(x) + ∇(p(t))(x) #FOR INS TEST
g(t) = x -> (∇⋅u(t))(x)
ud(t)=u(t)
...
##"ADD BELOW" - REAL VERSION
#Forcing terms & background grid
#f(t) = 0.0
#g(t) = 0.0
...
u_MRI_Name(i) = "u_MRI_$(i)"
u_MRI_Path(i) = join([DATAPATH, u_MRI_Name(i)], "/")

u_MRI_import(i) = CSV.read(u_MRI_Path(i))
u_MRI_values(i) = convert(Array,u_MRI_import(i).u_MRI)
u_MRI(i) = FEFunction(V2,u_MRI_values(i))
...
l((v,q)) = 
  ...
  ∫( ν * v⋅(n_Γn⋅∇(u_Γn(i*dt))) - (n_Γn⋅v)*p_Γn(i*dt) )dΓn
  ...
  @show sqrt(sum( ∫(l2(u_P))dΩ ))

  @show ulex2i = sqrt(sum( ∫(l2(u_ex))dΩ ))
  #@show upexl2i = sqrt(sum( ∫(l2(u_ex))dΩ ))
